...
Implementation follow-up: finding 1 is applied. The scanner's ordering test now covers descending allocated size and natural-number name ties, and all 127 focused Release scanner/sort tests pass. Finding 2 is the next approved change.
...
Cold-cache disks, network/removable volumes, cloud providers, and live SwiftUI interaction latency remain unmeasured. The candidate changes after finding 1 have not been implemented or benchmarked. Incremental rescans already localize disk reads, but their sequential deep-subtree rescans and whole-store splice remain separate follow-up profiling targets; this audit did not measure those paths.
