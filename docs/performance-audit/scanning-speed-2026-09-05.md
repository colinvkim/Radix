...
The path-reuse portion of finding 3 is also applied. Reusing each node's existing path reduced isolated node-preparation time by 30.6% for short ASCII paths and 44.7% for long Unicode paths. Complete scans had smaller, variable differences. The [path-reuse follow-up](/Users/colin/Programming/Radix/docs/performance-audit/path-reuse-2026-09-05.md) records the scope, paired measurements, and correctness checks. A subsequent [decoded-name reuse experiment](/Users/colin/Programming/Radix/docs/performance-audit/decoded-name-reuse-2026-09-05.md) found no convincing complete-scan benefit after balancing run order; that prototype was removed.

Finding 4 was also [profiled with a bounded overlap prototype](/Users/colin/Programming/Radix/docs/performance-audit/scan-overlap-2026-09-05.md). Wide-directory enumeration is substantial, but the simplified overlap experiment saved only about 12 ms with overlapping ranges. Scanner scheduling changes remain deferred; the opt-in benchmark and Debug leaf-preparation timer are retained.
...
Cold-cache disks, network/removable volumes, cloud providers, and live SwiftUI interaction latency remain unmeasured. Finding 5 remains unimplemented and unbenchmarked; finding 4 was profiled with a feasibility prototype and its scheduling change remains deferred. The decoded-native-name portion of finding 3 was benchmarked and rejected in the follow-up linked above. Incremental rescans already localize disk reads, but their sequential deep-subtree rescans and whole-store splice remain separate follow-up profiling targets; this audit did not measure those paths.
